    ...
    if request.user.is_authenticated:
        if request.user.is_superuser:
            ...
            if request.method == 'POST':
                ...
                handle_uploaded_file(request.FILES['file_upload'], user_information.id)
                user_information.photo = f'{user_information.id}.png'
...
def handle_uploaded_file(file, name):
    with open(f"search_user/static/img/{name}.png", "wb+") as destination:
